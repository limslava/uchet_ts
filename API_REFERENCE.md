...
## 🚗 Управление актами приёмки
- `GET /api/admin/vehicle-acts` - Список актов с пагинацией
...
- `GET /api/admin/vehicle-acts/:id/print` - Печать акта (⚠️ 404 ERROR)
- `GET /api/admin/vehicle-acts/:id/print-contract` - Печать договора (⚠️ 404 ERROR)
...
### Ошибка 404 - Эндпоинты печати
```bash
GET /api/admin/vehicle-acts/:id/print → 404 Not Found
GET /api/admin/vehicle-acts/:id/print-contract → 404 Not Found
Ошибка 500 - Обновление актов
bash
PUT /api/admin/vehicle-acts/:id → 500 Internal Server Error
Проблемы с данными
Фотографии не отображаются в панели админа

Комплектация не отображается в просмотре актов

🛠️ Статус разработки
Версия API: 2.1.0
Последнее обновление: 2025-09-08
Статус: В разработке (известные ошибки
...
### Решенные проблемы:
- ✅ **Удаление в справочниках**: Исправлена производительность удаления в справочниках "Направления", "Способы перевозки", "Локации"
- ✅ **Изменение пароля**: Полностью переработан механизм смены пароля пользователей
- ✅ **Новые справочники**: Добавлены API endpoints для водителей, ТС перевозчиков и контейнеров

### Новые эндпоинты:
- `GET /api/admin/additional-dictionaries/drivers` - Список водителей
- `POST /api/admin/additional-dictionaries/drivers` - Создание водителя
- `PUT /api/admin/additional-dictionaries/drivers/:id` - Обновление водителя
- `DELETE /api/admin/additional-dictionaries/drivers/:id` - Удаление водителя

- `GET /api/admin/additional-dictionaries/company-vehicles` - Список ТС перевозчиков
- `POST /api/admin/additional-dictionaries/company-vehicles` - Создание ТС перевозчика
- `PUT /api/admin/additional-dictionaries/company-vehicles/:id` - Обновление ТС перевозчика
- `DELETE /api/admin/additional-dictionaries/company-vehicles/:id` - Удаление ТС перевозчика

- `GET /api/admin/additional-dictionaries/containers` - Список контейнеров
- `POST /api/admin/additional-dictionaries/containers` - Создание контейнера
- `PUT /api/admin/additional-dictionaries/containers/:id` - Обновление контейнера
- `DELETE /api/admin/additional-dictionaries/containers/:id` - Удаление контейнера

### Оптимизации производительности:
- Добавлено кэширование запросов на 30 секунд
- Внедрены индексы базы данных для ускорения запросов
- Реализованы групповые запросы для подсчета связанных записей
- Добавлен механизм debounce для поиска на фронтенде

## ⚠️ Известные проблемы
- Запросы DELETE в новых справочниках могут занимать до 2000ms
- Медленные COUNT запросы при проверке связанных записей
- Необходима дополнительная оптимизация обработчиков кликов

### ⚠️ Известные проблемы
- **Пропадание панели навигации** в разделе справочников (в процессе исправления) - не решено
- Ошибки 404 для новых справочников (водители, ТС перевозчиков, контейнеры) - решено